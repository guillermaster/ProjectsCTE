...
function pieChart() {
  ...
  function startPullOut ( slice ) {
    ...
    var auxCell_1 = $('.chartData td:eq(' + (slice*4+2) + ')');
    var auxCell_2 = $('.chartData td:eq(' + (slice*4+3) + ')');
    labelCell.addClass('highlight');
    valueCell.addClass('highlight');
    auxCell_1.addClass('highlight');
    auxCell_2.addClass('highlight');
  }
  ...
  function drawSlice ( context, slice ) {
    ...
    if ( slice == currentPullOutSlice ) {
      ...
      //alert("rgb("+chartColours[slice].join(',')+")");
      context.textAlign = "center";
      context.font = pullOutLabelFont;
      
      try{
        context.fillText( chartData[slice]['label'], centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) );
        context.fillText( chartData[slice]['value'] + " (" + ( ( chartData[slice]['value'] / totalValue * 100 + .5 ) ).toFixed(2) +  "%)", centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) + 20 );
      }catch(e) {
        $('#aux').html("<span style='color: rgb(" + chartColours[slice].join(',') + "); font-weight: bold'>" + chartData[slice]['label'] + ": " + chartData[slice]['value'] + " (" + ( parseInt( chartData[slice]['value'] / totalValue * 100 + .5 ) ) +  "%)</span>");
      }
