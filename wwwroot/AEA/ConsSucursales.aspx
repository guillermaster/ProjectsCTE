<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  AutoEventWireup="true" CodeFile="ConsSucursales.aspx.cs" Inherits="ConsComercializadoras" %>
...
<DIV id="divContent"><asp:UpdateProgress id="UpdateProgress1" runat="server" AssociatedUpdatePanelID="UpdatePanel1" DisplayAfter="5" DynamicLayout="False">
        ...
        </asp:UpdateProgress> <DIV style="WIDTH: 481px" class="registerform"><asp:Label id="lblCodAutomotor" runat="server" Text="Criterio de búsqueda:"></asp:Label> <asp:DropDownList id="ddlBusqueda" runat="server" AutoPostBack="True" OnSelectedIndexChanged="ddlBusqueda_SelectedIndexChanged">
        </asp:DropDownList> <asp:DropDownList id="ddlBusqProvincia" runat="server" Visible="False" CssClass="field" Width="179px"></asp:DropDownList> <asp:TextBox id="txtBusqUsuario" runat="server" Visible="False"></asp:TextBox> <asp:TextBox id="txtBusqFecha" runat="server" Visible="False"></asp:TextBox> <cc1:CalendarExtender id="calExt" runat="server" Format="dd/MM/yyyy" TargetControlID="txtBusqFecha"></cc1:CalendarExtender> <asp:Button id="btnSearch" onclick="btnSearch_Click" runat="server" Text="Buscar" CssClass="button_horiz"></asp:Button> </DIV><BR /><BR /><BR /><asp:GridView id="gvSucursales" runat="server" OnSelectedIndexChanged="gvComercializadoras_SelectedIndexChanged" AutoGenerateColumns="False" EmptyDataText="No existe ninguna sucursal activa con el criterio de búsqueda seleccionado" CssClass="mGrid" PagerStyle-CssClass="pgr" AlternatingRowStyle-CssClass="alt">
<EmptyDataRowStyle ForeColor="Maroon"></EmptyDataRowStyle>
<Columns>
<asp:TemplateField><ItemTemplate>
        <asp:HiddenField ID="hdnCodSuc" Value='<%# Bind("id_det_comercializadora") %>' runat="server"></asp:HiddenField>
</ItemTemplate>
</asp:TemplateField>
...
<asp:TemplateField><ItemTemplate>
                        <asp:ImageButton ID="btnEdit" runat="server" OnClick="gvComercializadoras_Edit" ImageUrl="~/img/ico_edit.gif" ToolTip="Modificar sucursal" />
...
<asp:TemplateField><ItemTemplate>
                        <asp:ImageButton ID="btnDelete" runat="server" OnClientClick="return confirm('¿Está seguro que desea desactivar la sucursal?');" OnClick="gvComercializadoras_Delete" ImageUrl="~/img/ico_delete.gif" ToolTip="Desactivar sucursal" />
