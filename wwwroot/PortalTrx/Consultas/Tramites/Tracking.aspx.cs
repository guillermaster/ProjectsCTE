...
public partial class Consultas_Tramites_Tracking : System.Web.UI.Page
{
    ...
    protected void ConsultarCEPsTramites(string identificacion)
    {
        ...
        try
        {
            DataTable dtCEPs = oCEP.ConsultaCEPTramitexUsuario(identificacion, false);
            ...
            if (gvTrackTramites.Rows.Count == 0)
            {
                if (string.IsNullOrWhiteSpace(oCEP.Error))
                    pnlContent.Controls.Add(HtmlWriter.Messages.PrintInfoMessage("Usted no tiene ningún trámite pendiente."));
                else
    ...
    protected void CargarDatosAdicionalesTramite()
    {
        ...
        DataTable dtTramite = oCEP.ConsultaDatosTramite(((HiddenField)gvTrackTramites.SelectedRow.FindControl("hdnIdTramite")).Value);
